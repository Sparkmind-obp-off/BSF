...
export class FactoryApplication {
  ...
  validate(id) {
    const b=this.require(id), stages=[validateComposition(b,this.catalog),validateConfiguration(b),validateConnections(b)];
    ...
    const c=result.status==='PASS'?advanceState(b,'VALIDATED'):b;
