...
test('custom build follows canonical lifecycle through validation',()=>{const app=makeApp();let build=makeConfiguredBuild(app);assert.equal(build.state,'CONFIGURED');({build}=app.validate(build.id));assert.equal(build.state,'VALIDATED');assert.equal(build.validations.at(-1).status,'PASS');assert.equal(build.validationRevision,build.validations.at(-1).revision);assert.ok(build.validationRevision<build.revision);});
test('invalid composition is blocked',()=>{const app=makeApp();let build=app.create({name:'Broken',source:'modules'});build=app.define(build.id);const result=app.compose(build.id,['sales-pipeline']);assert.equal(result.gate.status,'BLOCK');assert.equal(app.get(build.id).state,'DEFINED');});
test('validation cannot start before configuration',()=>{const app=makeApp();let build=app.create({name:'Too Early',source:'custom'});build=app.define(build.id);app.compose(build.id,['customer-records']);assert.throws(()=>app.validate(build.id),/BUILD_NOT_READY_FOR_VALIDATION/);});
test('material configuration change invalidates previous validation for deployment',()=>{const app=makeApp();let build=makeConfiguredBuild(app);({build}=app.validate(build.id));assert.equal(build.state,'VALIDATED');({build}=app.configure(build.id,{currency:'USD'}));assert.equal(build.state,'CONFIGURED');assert.equal(build.validations.length,0);assert.equal(build.validationRevision,null);assert.throws(()=>app.prepareDeployment(build.id),/BUILD_NOT_VALIDATED/);});
